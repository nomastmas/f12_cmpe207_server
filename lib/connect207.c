...
int connect207_check_3way_response_packet(int tcp_block_index_in, int tcp_state_in )
{
	...
	switch(tcp_state_in)
	{
		...
		case SYN_RCVD://CONNECT207_SYN_ACK: //this is useful for SYN_ACK. 
		{
		...
		case ESTABLISHED://CONNECT207_ACK: //last step in 3-way handshaking
		{
			/*Ack Num*/ 
			if(gTcp_Block[tcp_block_index_in].pTcpH->ack_num != gTcp_Block[tcp_block_index_in].pSeq->send_current_seq_number + 1) //the (received sequencenumber + 1) 
			{
		...
		default:
		{
			printf("Default error for connect207_check_3way_response_packet() %d\n",tcp_state_in);
			return TCP207_CONNECT_ERROR_WRONG_3WAY_RESPONSE;
			break;	
		}
...
/*Src becomes dest and vice versa*/
	...
	switch(tcp_state_in)
	{
		...
		case SYN_RCVD://CONNECT207_SYN_ACK: //when client sends SYN to server then this will not be useful to send. But for server when it receives SYN it has to send SYN ACK then this is useful. 
		{
			/*Seq Num*/
...
/*connect207() function will establish an active connection to a remore server. */
...
//	int state = 0;
	...
	do{
		...
		retValS = get_tcp_state(aTcpState, *(gTcp_Block[tcp_block_index_in].pTcpH), msg); //SB:The TCP dtate diagram us changing
		aTcpState = retValS;
		printf ("@@@@@@@TCPState=%d\n",retValS);
		if(retValS == CLOSED)
		{
			printf ("Error: sendto() retValS == -1 %s\n",strerror(errno));
			return TCP207_ERROR_TCPSTATE_MISBEHAVE;	
		}

		printf ("@@@@@@@ TCPState=%d packet @@@@@@\n", aTcpState);
...
/* After sending ACK packet, exit while loop*/
		if((aTcpState == CLOSED) | (aTcpState == ESTABLISHED))
		{
			aExit = 1;
			break;
		...
		/*Call TCP state machine to move ahead the state*/
	
		retValS1 = get_tcp_state(aTcpState1, *(gTcp_Block[tcp_block_index_in].pTcpH), msg1); //SB:The TCP dtate diagram us changing
		aTcpState1 = retValS1;
		printf ("@@@@@@@TCPState=%d\n",retValS1);
		if(retValS1 == CLOSED)
		{
			printf ("Error: sendto() retValS1 == -1 %s\n",strerror(errno));
			return TCP207_ERROR_TCPSTATE_MISBEHAVE;	
		}
		


		//Check whether the packet received is correct SYNACK packet IF YES then continue
		retVal = connect207_check_3way_response_packet(tcp_block_index_in,aTcpState1);
		if(retVal != TCP207_SUCCESS)
		{
			printf("Error: Returned from connect207_check_3way_retesponse_packet()\n");
			return retVal;//SB: Handle retransmission 
