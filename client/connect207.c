...
int connect207_check_3way_response_packet(int tcp_block_index_in, int tcp_state_in )
{
	...
	switch(tcp_state_in)
	{
		...
		case CONNECT207_SYN_ACK: //this is useful for SYN_ACK. 
		{
			...
			printf("should receive: %d\n", gTcp_Block[tcp_block_index_in].pSeq->send_current_seq_number + 1);
			/*Ack Num*/ 
...
/*Src becomes dest and vice versa*/
	tmp16 = 2102;//gTcp_Block[tcp_block_index_in].pTcpH->source_port;
	gTcp_Block[tcp_block_index_in].pTcpH->source_port =9000;// gTcp_Block[tcp_block_index_in].pTcpH->dest_port ;
